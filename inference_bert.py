...
import argparse
from bert_model import SentimentClassifierBERT
...
tokenizer = BertTokenizer.from_pretrained(config.BERT_PRE_TRAINED_MODEL)
model = SentimentClassifierBERT(config.NUM_LABELS, model_config).to(config.device)
checkpoint = torch.load(config.BERT_MODEL_PATH)
